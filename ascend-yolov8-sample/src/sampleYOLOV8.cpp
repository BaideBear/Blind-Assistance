                               ...
                               string imagePath, size_t imageIndex, bool release)
{
    ...
    // 打印信息：过滤后的框数量
    ACLLITE_LOG_INFO("成功过滤置信度阈值 > %f 的框，框的数量为 %ld", confidenceThreshold, boxes.size());
    ...
    while (boxes.size() != 0)
    {
        ...
        while (boxes.size() > index)
        {
            ...
            // 通过 maxLength * boxes[0].classIndex 平移点以避免两个不同类的框重叠
    ...
    // 打印信息：NMS过滤后的结果数量
    ACLLITE_LOG_INFO("成功过滤NMS阈值 > %f 的框，结果数量为 %ld", NMSThreshold, result.size());
    ...
    // 打开文件以追加方式写入
    std::ofstream outFile("ans.txt", std::ios::app);
    ...
    for (size_t i = 0; i < result.size(); ++i) {
        ...
        // 打印检测成功的信息
        ACLLITE_LOG_INFO("对象检测 [%s] 成功", markString.c_str());
...
int main()
{
    ...
    std::ofstream outFile("ans.txt", std::ios::trunc);
    outFile.close();//清空文件
